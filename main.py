...
def composite(**kwargs):
    ...
    if len(name) < 14:
        ...
        while name != "":
            ...
            z = Image.new("RGBA", img.size)
            z.paste(letter, (n + 80 + int(int(stat["xoffset"]) / 2), 27 + int(int(stat["yoffset"]) / 2)))
        ...
        while name != "":
            ...
            letter = alphabet.crop((int(stat["x"]), int(stat["y"]), int(stat["x"]) + int(stat["width"]),
                                    int(stat["y"]) + int(stat["height"])))
            s = letter.size
            letter = letter.resize((int(s[0] / 3), int(s[1] / 3)), 3)

            z = Image.new("RGBA", img.size)
            z.paste(letter, (n + 80 + int(int(stat["xoffset"]) / 3), 35 + int(int(stat["yoffset"]) / 3)))

            timed.append(z)
            name = name[1:]
            n += int(int(stat["xadvance"]) / 3)
...
def dual_composite(**kwargs):
    ...
    # 1.5 faction
    if kwargs["faction"] in CARDS:
        img.paste(Image.open(kwargs["faction"] + ".png").crop((224, 0, CARDS[kwargs["faction"]][2], CARDS[kwargs["faction"]][3])),
                  box=(224, 0))
    #, CARDS[kwargs["faction"]][2], CARDS[kwargs["faction"]][3]
    else:
        img.paste(Image.open(kwargs["faction"] + ".png").resize((437, 552)),
                  box=(224, 0))
    # 2 shiny
    if kwargs["shiny"]:
        img = img.resize((437, 552), resample=0)
...
def dual_gradient(**kwargs):
    ...
    img2 = Image.new("RGBA", (437, 552))
    # 1.5 faction
    if kwargs["faction"] in CARDS:
        img2.paste(Image.open(kwargs["faction"] + ".png"), box=(0, 0))
    else:
        img2.paste(Image.open(kwargs["faction"] + ".png").resize((437, 552)), box=(0, 0))

    ist =img.copy()
    for x in range(15, 436):
        i = ist.crop((x, 0, x+1, 545))
        i2 = img2.crop((x, 0, x+1, 545))

        img.paste(Image.blend(i, i2, x/436), (x, 0))
